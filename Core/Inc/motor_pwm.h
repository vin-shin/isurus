/**
  ...
  *
  *          Only output 1 of each pair is given a set and reset source. Output
  *          2 is generated by the timer's dead-time unit from output 1, which
  *          is what makes the pair genuinely complementary - two independently
  *          programmed waveforms that happen to be inverses of each other
  *          would have no guaranteed dead band between them at all.
  *
...
/* Duty per phase in counts, 0..period, referred to the HIGH side. Applied
 * through the preload registers so all three take effect on the same period
 * boundary. The low side of each leg follows from the dead-time unit. */
